...
unsafe extern "C" fn ts_parser__log(mut self_0: *mut TSParser) {
    ...
    if !(*self_0).dot_graph_file.is_null() {
        fprintf(
            ...
            b"graph {\nlabel=\"\x00" as *const u8 as *const os::raw::c_char,
        );
        ...
        while *c as os::raw::c_int != 0 as os::raw::c_int {
            ...
            fputc(*c as os::raw::c_int, (*self_0).dot_graph_file);
            c = c.offset(1)
        }
        fprintf(
            (*self_0).dot_graph_file,
...
unsafe extern "C" fn ts_parser__breakdown_top_of_stack(
    ...
    loop {
        ...
        while i < pop.size {
            ...
            if (0 as os::raw::c_int as u32) < slice.subtrees.size {
            } else {
                __assert_fail(
                    b"(u32)0 < (&slice.subtrees)->size\x00" as *const u8 as *const os::raw::c_char,
                    b"lib/src/parser.c\x00" as *const u8 as *const os::raw::c_char,
                    154 as os::raw::c_int as os::raw::c_uint,
            ...
            if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
                snprintf(
                    (*self_0).lexer.debug_buffer.as_mut_ptr(),
                    1024,
                    b"breakdown_top_of_stack tree:%s\x00" as *const u8 as *const os::raw::c_char,
        ...
        && ts_subtree_parse_state(tree) as os::raw::c_int != state as os::raw::c_int
    {
        if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
            snprintf(
                (*self_0).lexer.debug_buffer.as_mut_ptr(),
                1024,
                b"state_mismatch sym:%s\x00" as *const u8 as *const os::raw::c_char,
            ...
            if !skipped_error {
                if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
                    snprintf(
                        (*self_0).lexer.debug_buffer.as_mut_ptr(),
                        1024,
                        b"skip_unrecognized_character\x00" as *const u8 as *const os::raw::c_char,
                    );
                    ts_parser__log(self_0);
                }
    ...
    if skipped_error {
        ...
        if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
            snprintf(
                (*self_0).lexer.debug_buffer.as_mut_ptr(),
                1024,
                b"lexed_lookahead sym:%s, size:%u\x00" as *const u8 as *const os::raw::c_char,
                ts_language_symbol_name((*self_0).language, ts_subtree_symbol(result)),
                ts_subtree_total_size(result).bytes,
            );
            ts_parser__log(self_0);
        ...
        if byte_offset > position {
            if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
                snprintf(
                    (*self_0).lexer.debug_buffer.as_mut_ptr(),
                    1024,
                    b"before_reusable_node symbol:%s\x00" as *const u8 as *const os::raw::c_char,
                    ts_language_symbol_name((*self_0).language, ts_subtree_symbol(result)),
                );
                ts_parser__log(self_0);
            }
            ...
            if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
                snprintf(
                    (*self_0).lexer.debug_buffer.as_mut_ptr(),
                    1024,
                    b"past_reusable_node symbol:%s\x00" as *const u8 as *const os::raw::c_char,
                    ts_language_symbol_name((*self_0).language, ts_subtree_symbol(result)),
                );
                ts_parser__log(self_0);
            }
            ...
            if !reason.is_null() {
                if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
                    snprintf(
                        (*self_0).lexer.debug_buffer.as_mut_ptr(),
                        1024,
                        b"cant_reuse_node_%s tree:%s\x00" as *const u8 as *const os::raw::c_char,
                        reason,
                        ts_language_symbol_name((*self_0).language, ts_subtree_symbol(result)),
                    );
                    ts_parser__log(self_0);
                ...
                if !ts_parser__can_reuse_first_leaf(self_0, *state, result, table_entry) {
                    ...
                    if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
                        snprintf(
                            (*self_0).lexer.debug_buffer.as_mut_ptr(),
                            1024,
                            b"reuse_node symbol:%s\x00" as *const u8 as *const os::raw::c_char,
                            ts_language_symbol_name((*self_0).language, ts_subtree_symbol(result)),
                        );
                        ts_parser__log(self_0);
                    }
...
unsafe extern "C" fn ts_parser__accept(
    ...
    if ts_subtree_is_eof(lookahead) {
        ...
        __assert_fail(
            b"ts_subtree_is_eof(lookahead)\x00" as *const u8 as *const os::raw::c_char,
            b"lib/src/parser.c\x00" as *const u8 as *const os::raw::c_char,
            863 as os::raw::c_int as os::raw::c_uint,
    ...
    while i < pop.size {
        ...
        while j.wrapping_add(1 as os::raw::c_int as os::raw::c_uint) > 0 as os::raw::c_int as os::raw::c_uint {
            ...
            if !ts_subtree_extra(child) {
                if !child.data.is_inline() {
                    ...
                    __assert_fail(
                        b"!child.data.is_inline\x00" as *const u8 as *const os::raw::c_char,
                        b"lib/src/parser.c\x00" as *const u8 as *const os::raw::c_char,
                        874 as os::raw::c_int as os::raw::c_uint,
                        (*::std::mem::transmute::<&[u8; 58], &[os::raw::c_char; 58]>(
                            b"void ts_parser__accept(TSParser *, StackVersion, Subtree)\x00",
                        ))
                        .as_ptr(),
        ...
        if !root.ptr.is_null() {
            ...
            __assert_fail(
                b"root.ptr\x00" as *const u8 as *const os::raw::c_char,
                b"lib/src/parser.c\x00" as *const u8 as *const os::raw::c_char,
                892 as os::raw::c_int as os::raw::c_uint,
                (*::std::mem::transmute::<&[u8; 58], &[os::raw::c_char; 58]>(
                    b"void ts_parser__accept(TSParser *, StackVersion, Subtree)\x00",
                ))
                .as_ptr(),
...
unsafe extern "C" fn ts_parser__handle_error(
    ...
    while i < version_count {
        ...
        if did_merge {
            ...
            __assert_fail(
                b"did_merge\x00" as *const u8 as *const os::raw::c_char,
                b"lib/src/parser.c\x00" as *const u8 as *const os::raw::c_char,
                1074 as os::raw::c_int as os::raw::c_uint,
                (*::std::mem::transmute::<&[u8; 65], &[os::raw::c_char; 65]>(
    ...
    while i < pop.size {
            ...
            != goal_state as os::raw::c_int
        {
            ...
            if error_trees.size > 0 as os::raw::c_int as os::raw::c_uint {
                if error_trees.size == 1 as os::raw::c_int as os::raw::c_uint {
                } else {
                    __assert_fail(b"error_trees.size == 1\x00" as *const u8 as
                                      *const os::raw::c_char,
                                  b"lib/src/parser.c\x00" as *const u8 as
                                      *const os::raw::c_char,
                                  1108 as os::raw::c_int as os::raw::c_uint,
                                  (*::std::mem::transmute::<&[u8; 85],
                                                            &[os::raw::c_char; 85]>(b"_Bool ts_parser__recover_to_state(TSParser *, StackVersion, unsigned int, TSStateId)\x00")).as_ptr());
                }
...
unsafe extern "C" fn ts_parser__recover(
    ...
    if ts_subtree_is_eof(lookahead) {
        if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
            snprintf(
                (*self_0).lexer.debug_buffer.as_mut_ptr(),
                1024,
                b"recover_eof\x00" as *const u8 as *const os::raw::c_char,
            );
            ts_parser__log(self_0);
        }
    ...
    if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
        snprintf(
            (*self_0).lexer.debug_buffer.as_mut_ptr(),
            1024,
            b"skip_token symbol:%s\x00" as *const u8 as *const os::raw::c_char,
        ...
        if (*self_0).operation_count == 0 as os::raw::c_int as os::raw::c_uint
            && (!(*self_0).cancellation_flag.is_null()
                ...
                || !clock_is_null((*self_0).end_clock)
                    && clock_is_gt(clock_now(), (*self_0).end_clock) as os::raw::c_int != 0)
        {
            ts_subtree_release(&mut (*self_0).tree_pool, lookahead);
            return 0 as os::raw::c_int != 0;
        }
                        ...
                        if action.params.c2rust_unnamed.extra() {
                            ...
                            if state as os::raw::c_int == 0 as os::raw::c_int {
                                    ...
                                    || !(*self_0).dot_graph_file.is_null()
                                {
                                    snprintf(
                                        (*self_0).lexer.debug_buffer.as_mut_ptr(),
                                        1024,
                                        b"shift_extra\x00" as *const u8 as *const os::raw::c_char,
                                    );
                                    ts_parser__log(self_0);
                                }
                                ...
                                || !(*self_0).dot_graph_file.is_null()
                            {
                                snprintf(
                                    (*self_0).lexer.debug_buffer.as_mut_ptr(),
                                    1024,
                                    b"shift state:%u\x00" as *const u8 as *const os::raw::c_char,
                ...
                2 => {
                    if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
                        snprintf(
                            (*self_0).lexer.debug_buffer.as_mut_ptr(),
                            1024,
                            b"accept\x00" as *const u8 as *const os::raw::c_char,
                        );
                        ts_parser__log(self_0);
                    }
                    ts_parser__accept(self_0, version, lookahead);
        ...
        if last_reduction_version != -(1 as os::raw::c_int) as StackVersion {
            ...
            if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
                snprintf(
                    (*self_0).lexer.debug_buffer.as_mut_ptr(),
                    1024,
                    b"detect_error\x00" as *const u8 as *const os::raw::c_char,
                );
                ts_parser__log(self_0);
            }
                ...
                if !has_unpaused_version && (*self_0).accept_count < MAX_VERSION_COUNT {
                    if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
                        snprintf(
                            (*self_0).lexer.debug_buffer.as_mut_ptr(),
                            1024,
                            b"resume version:%u\x00" as *const u8 as *const os::raw::c_char,
                            i_0,
                        );
                        ts_parser__log(self_0);
                    }
    ...
    if made_changes {
        if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
            snprintf(
                (*self_0).lexer.debug_buffer.as_mut_ptr(),
                1024,
                b"condense\x00" as *const u8 as *const os::raw::c_char,
            );
            ts_parser__log(self_0);
        }
        if !(*self_0).dot_graph_file.is_null() {
...
pub unsafe extern "C" fn ts_parser_print_dot_graphs(
    ...
    if fd >= 0 as os::raw::c_int {
        (*self_0).dot_graph_file = fdopen(fd, b"a\x00" as *const u8 as *const os::raw::c_char)
    } else {
        (*self_0).dot_graph_file = 0 as *mut FILE
    };
}
...
pub unsafe extern "C" fn ts_parser_parse(
    ...
    if ts_parser_has_outstanding_parse(self_0) {
        if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
            snprintf(
                (*self_0).lexer.debug_buffer.as_mut_ptr(),
                1024,
                b"resume_parsing\x00" as *const u8 as *const os::raw::c_char,
            );
            ts_parser__log(self_0);
        }
        ...
        if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
            snprintf(
                (*self_0).lexer.debug_buffer.as_mut_ptr(),
                1024,
                b"parse_after_edit\x00" as *const u8 as *const os::raw::c_char,
            );
            ts_parser__log(self_0);
        }
        if !(*self_0).dot_graph_file.is_null() {
        ...
        while i < (*self_0).included_range_differences.size {
            ...
            if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
                snprintf(
                    (*self_0).lexer.debug_buffer.as_mut_ptr(),
                    1024,
                    b"different_included_range %u - %u\x00" as *const u8 as *const os::raw::c_char,
                    (*range).start_byte,
                    (*range).end_byte,
                );
                ts_parser__log(self_0);
        ...
        if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
            snprintf(
                (*self_0).lexer.debug_buffer.as_mut_ptr(),
                1024,
                b"new_parse\x00" as *const u8 as *const os::raw::c_char,
            );
            ts_parser__log(self_0);
        }
    }
    ...
    if (*self_0).lexer.logger.log.is_some() || !(*self_0).dot_graph_file.is_null() {
        snprintf(
            (*self_0).lexer.debug_buffer.as_mut_ptr(),
            1024,
            b"done\x00" as *const u8 as *const os::raw::c_char,
        );
        ts_parser__log(self_0);
    }
    if !(*self_0).dot_graph_file.is_null() {
