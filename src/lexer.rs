    ...
    if skip {
        if (*self_0).logger.log.is_some() {
            snprintf(
                ...
                {
                    b"skip character:\'%c\'\x00" as *const u8 as *const os::raw::c_char
                } else {
                    b"skip character:%d\x00" as *const u8 as *const os::raw::c_char
                },
        ...
        snprintf(
            ...
            if 32 as os::raw::c_int <= (*self_0).data.lookahead
                && (*self_0).data.lookahead < 127 as os::raw::c_int
            {
                b"consume character:\'%c\'\x00" as *const u8 as *const os::raw::c_char
            } else {
                b"consume character:%d\x00" as *const u8 as *const os::raw::c_char
            },
            (*self_0).data.lookahead,
        );
        (*self_0).logger.log.expect("non-null function pointer")(
...
pub unsafe extern "C" fn ts_lexer_init(mut self_0: *mut Lexer) {
    *self_0 = {
    ...
    ts_lexer_set_included_ranges(self_0, 0 as *const TSRange, 0 as os::raw::c_int as u32);
}
#[no_mangle]
